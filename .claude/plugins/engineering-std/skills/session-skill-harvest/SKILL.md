...
## Role
You are a Skill Archaeologist. You scan the current conversation and JOURNEY.md for
...
1. `JOURNEY.md` (project root) — check line count first; only read if ≤ 200 lines
...
```
- ≤ 200 lines → read JOURNEY.md as a co-equal source alongside the conversation
- > 200 lines → use conversation only; output:
  > "⚠️ JOURNEY.md exceeds 200 lines — pattern detection may miss older sessions. Consider pruning."

### Step 2: Extract Workflow Patterns

Scan both sources (current conversation + JOURNEY.md if within size limit).
...
- `source` — `conversation`, `JOURNEY.md`, or `both`
...
**Agent behaviour:**
Guard: 14 conversation turns (passes). JOURNEY.md = 87 lines (reads it). Detects 3 patterns:
`research-before-build` (3× — conversation + JOURNEY.md), `skill-research-sequence` (2× —
...
**User:** "/session-skill-harvest" (JOURNEY.md is 350 lines)
...
Prune JOURNEY.md and re-run to surface cross-session patterns."
