...
| [your-cloudflare] edge 503 at `project-life-130.or-infra.com` | `curl` to `or-infra.com` returns 503 "DNS cache overflow"; [your-telegram] probes via bot are **unaffected** | [your-cloudflare] CNAME/DNS cache issue on the [your-cloudflare] side — independent of N8N/[your-railway] | `configure-telegram-n8n.yml` resolves `N8N_DOMAIN` from [your-railway] GraphQL and bakes the **direct [your-railway] hostname** into both `WEBHOOK_URL` ([your-telegram] `setWebhook`) and `AGENT_ROUTER_URL` (Router dispatch). The [your-telegram] → N8N path bypasses [your-cloudflare] entirely, so an edge 503 is **not a Stage 7 blocker**. File as a separate Level-3 follow-up (update [your-cloudflare] CNAME target or open a [your-cloudflare] support ticket) |
